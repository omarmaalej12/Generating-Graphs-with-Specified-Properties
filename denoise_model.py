...
class DenoiseNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_layers, n_cond, d_cond, text_emb_dim=768): # Added text_emb_dim
        super(DenoiseNN, self).__init__()
        self.n_layers = n_layers
        self.n_cond = n_cond
        self.text_emb_dim = text_emb_dim # Added text_emb_dim

        self.cond_mlp = nn.Sequential(
            nn.Linear(text_emb_dim, d_cond),  # Modified input dimension
            nn.ReLU(),
        ...
        mlp_layers = [nn.Linear(input_dim+d_cond, hidden_dim)] + [nn.Linear(hidden_dim+d_cond, hidden_dim) for i in range(n_layers-2)] # Modified to add text embedding
    ...
    def forward(self, x, t, cond):
        cond= cond.view(-1, self.text_emb_dim) # Modified to use the text embedding
        cond = self.cond_mlp(cond) # Modified to use the text embedding
        t = self.time_mlp(t)
        for i in range(self.n_layers-1):
           x = torch.cat((x, cond), dim=1) # Concatenate text embeddings
...
@torch.no_grad()
def sample(model, cond, latent_dim, timesteps, betas, batch_size): # Modified cond
    return p_sample_loop(model, cond, timesteps, betas, shape=(batch_size, latent_dim)) # Modified cond