...
def preprocess_dataset(dataset, n_max_nodes, spectral_emb_dim):
    ...
    model=AutoModel.from_pretrained('jinaai/jina-embeddings-v3', code_revision='da863dd04a4e5dce6814c6625adfba87b83838aa',
    ...
    if dataset == 'test':
        ...
        if os.path.isfile(filename):
            ...
            for line in fr:
                ...
                feats_stats = model.encode(desc,truncate_dim=512,task="text-matching")
        ...
        if os.path.isfile(filename):
            ...
            for fileread in tqdm(files):
                ...
                line=file.read()
                line=line.strip()
                feats_stats = model.encode(line,truncate_dim=512,task="text-matching")
                feats_stats = torch.FloatTensor(feats_stats).unsqueeze(0)
